...
const RfqForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      attachments.forEach(file => data.append('attachment', file)); // ✅ match schema
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-white p-8 rounded shadow-md w-full max-w-lg mx-auto">
      ...
      <div className="mb-4">
        ...
        <input id="title" name="title" type="text" className="w-full border rounded px-3 py-2"
      ...
      <div className="mb-4">
        ...
        <textarea id="description" name="description" className="w-full border rounded px-3 py-2"
          rows={4} value={formData.description} onChange={handleChange} disabled={loading} required />
      </div>

      <div className="mb-4">
        <label htmlFor="requestType" className="block mb-1 font-semibold">Request Type</label>
        <select id="requestType" name="requestType" className="w-full border rounded px-3 py-2"
          value={formData.requestType} onChange={handleChange} disabled={loading} required>
      ...
      <div className="mb-4">
        ...
        <input id="budget" name="budget" type="number" className="w-full border rounded px-3 py-2"
          value={formData.budget} onChange={handleChange} disabled={loading} required />
      </div>
      ...
      <div className="mb-4">
        ...
        <select id="category" name="category" className="w-full border rounded px-3 py-2"
          value={formData.category} onChange={handleChange} disabled={loading} required>
      ...
      <div className="mb-4">
        ...
        <input id="deadline" name="deadline" type="date" className="w-full border rounded px-3 py-2"
          value={formData.deadline} onChange={handleChange} disabled={loading} required />
      </div>

      <div className="mb-6">
        <label htmlFor="attachment" className="block mb-1 font-semibold">Attachments (optional)</label>
        <input id="attachment" name="attachment" type="file" multiple onChange={handleFileChange}
          disabled={loading} className="w-full border rounded px-3 py-2" />
      </div>
