  ...
  return (
    <div className="bg-white p-8 rounded shadow-md max-w-3xl mx-auto">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mb-6">
        <div><p className="font-semibold">Request Type:</p><p>{rfq.requestType}</p></div>
        <div><p className="font-semibold">Category:</p><p>{rfq.category}</p></div>
        <div><p className="font-semibold">Budget:</p><p>₹{rfq.budget}</p></div>
        <div><p className="font-semibold">Deadline:</p><p>{new Date(rfq.deadline).toLocaleDateString()}</p></div>
        <div><p className="font-semibold">Status:</p><p>{rfq.status}</p></div>
      </div>
            ...
            <tbody>
              {quotations.map((q) => (
                <tr key={q._id} className="border-b hover:bg-gray-50">
                  ...
                  <td className="p-3">₹{q.price || 'N/A'}</td>
